...
async function accountPasswordUpdate(req, res, next) {
  let nav = await utilities.getNav();
  ...
  if (updateResult) {
    ...
    req.flash("notice", "Sorry, the password was not successfully updated.");
    res.status(501).render("./account/edit-account", {
      title: "Edit Account",
      nav,
      errors: null,
...
async function accountDelete(req, res, next) {
  ...
  console.log(`*********DELETEACCOUNTCONTROLLER${account_id}`);
